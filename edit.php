...
if (!empty($action)) {
    ...
    if ($action == 'edit' and $linkid) {
        ...
        $link_settings = $DB->get_record('linkset_links', array('id' => $linkid));
        $current->urltype = $link_settings->urltype;
...
$current = file_prepare_standard_filemanager($current, 'fileurl', $attachmentoptions, $context, 'mod_linkset', 'file', $current->linkid);
